...
const PI = 3.14159
# PI = 3.14 , Нельзя менять значение константы

# Что происходит с типами глобальных переменных ниже, какого типа `c` и почему?
a = 1
b = 2.0
c = a + b
# Тип 'с' - Float64, потому что множество континуальных чисел включает в себя множество целых, выбирается тип, который может хранить в себе результат без потери данных

# Что теперь произошло с переменной а? Как происходит биндинг имен в Julia?
a = "foo"
# 'a' имеет тип String и значение "foo", переменные в julia, как и в множестве динамических ЯП, это ссылки на данные/объекты

# Что происходит с глобальной переменной g и почему? Чем ограничен биндинг имен в Julia?
g::Int = 1
g = "hi"
# Выдастся ошибка, т.к. в данном случае биндинг имен ограничен конкретным типом при объявлении переменной
...
greet()
# Функция выполнится без ошибки, т.к. мы в первую очередь будем обращаться к ближайшей локальной переменной внутри блока видимости, а потом только к глобальной, никаких ограничений на локальной при этом не стоит
...
z
#= 
Присвоение - это операция над именем, z = v (копирование ссылки), сам объект не меняется.
Мутация - это изменение объекта.
В данном случае мы сделали копию ссылки на массив из 3 чисел, потом мутировали значение по этим ссылкам, после присвоили для имени v ссылку на объект "hello", но массив мы все еще можем получить по ссылке z
=#
